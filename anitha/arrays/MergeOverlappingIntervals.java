...
current = (1,5)
result = []
Step 2: Compare with next interval (3,7)
Check: 3 <= 5 → overlap ✅

Merge:

current.end = max(5,7) = 7
Now:
current = (1,7)
result = []
Step 3: Compare with next interval (4,6)
Check: 4 <= 7 → overlap ✅

Merge:

current.end = max(7,6) = 7
Now:
current = (1,7)
result = []
Step 4: Compare with next interval (6,8)
Check: 6 <= 7 → overlap ✅

Merge:

current.end = max(7,8) = 8
Now:
current = (1,8)
result = []
Step 5: End of loop
...
Think of it like stretching one interval:

Keep expanding current as long as overlaps happen
Only “lock it in” when a gap appears
