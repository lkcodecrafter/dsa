...
void clearBit(int &n, int pos) {
    ...
    // 2. ~(1 << pos) flips all bits (bitwise NOT), making the bit at 'pos' 0 and all others 1.
    // 3. n & ... performs bitwise AND, which keeps all other bits of 'n' the same but forces the bit at 'pos' to 0.
    //n = n & ~(1 << pos);
