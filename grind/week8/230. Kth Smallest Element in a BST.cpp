...
public:
    ...
    int solve(TreeNode* root, int &k){
        ...
        k=k-1; // why k=k-1 ? because we are processing the current node, 
        //so we decrement k by 1 to indicate that we have processed one more node in our search for the 
        //kth smallest element. If k becomes 0 after this decrement, it means the current node is the kth smallest element, and we can return its value.
        if(k==0){
            return root->val;
        }
