...
public:
    ...
    void DFS(vector<vector<char>>& board, int i, int j) {
        ...
        // Mark this 'O' as 'Safe' (using a temporary character)
        board[i][j] = '#';
    ...
    void solve(vector<vector<char>>& board) {
        ...
        // 1. Run DFS from the 'O's on the TOP and BOTTOM borders
        for (int j = 0; j < n; j++) {
            if (board[0][j] == 'O') DFS(board, 0, j);
            if (board[m - 1][j] == 'O') DFS(board, m - 1, j);
        }

        // 2. Run DFS from the 'O's on the LEFT and RIGHT borders
        for (int i = 0; i < m; i++) {
