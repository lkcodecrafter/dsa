...
Time Complexity: O(n)
- The in-order traversal visits each node exactly once, where n is the number of
nodes in the binary tree.

Space Complexity: O(h)
- The space complexity is determined by the recursion stack during the in-order
traversal, where h is the height of the binary tree.
- In the worst case, the height of the binary tree can be n for a skewed tree,
...
bool solve(TreeNode *root, long low, long high) {
  ...
  if (root->val >= high || root->val <= low)
    return false;
  return solve(root->left, low, (long)root->val) &&
         solve(root->right, (long)root->val, high);
}
