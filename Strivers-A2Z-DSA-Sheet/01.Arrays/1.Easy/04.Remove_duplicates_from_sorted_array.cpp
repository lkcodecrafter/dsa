...
k, to get accepted, you need to do the following things: Change the array nums
such that the first k elements of nums contain the unique elements in the order
they were present in nums initially. The remaining elements of nums are not
...
/*
APPROACH:-
-> The idea, is to use keep a pointer k which signifies that upto here the array
is sorted
-> Now travese the entire array and if arr[k]!=arr[j] that is arr[j] is a unique
value hence it should be included so increment the k and swap arr[k] with arr[j]
